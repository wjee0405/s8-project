...
import net.springboot.synpulse8challenges.model.Country;
...
public class AccountOpsImpl {
    ...
    public ResponseEntity<ResponseObject> findAllCountries() {
        List<String> countries = new ArrayList<>();
        List<Country> enumList = EnumUtils.getEnumList(Country.class);
        for (Country country : enumList) {
            countries.add(country.getCountry());
        }

        return ResponseUtility.buildResponse(null, HttpStatus.OK, countries);
    }
    ...
    public Account createAccountInCurrency(String userId, String country) {
        ...
        account.setCurrency((Country.valueOf(country).getCurrency()));
    ...
    private String generateNewAccount(String country) {
        ...
        String tempAccount = Country.valueOf(country).getAccountPrefix();
        String accountNo = "";
